...
def make_stream_button(file_id: str):
    return InlineKeyboardMarkup([[InlineKeyboardButton("🚀 Stream & Fast Download", callback_data=f"generate_stream_link:{file_id}")]])
# --- end helper ---
...
async def start(client, message):
    ...
    if len(message.command) != 2:
        ...
        reply_markup = InlineKeyboardMarkup(buttons)
        m=await message.reply_sticker("CAACAgUAAxkBAAKoq2flfc_kyjwMCA93RLXjJjgL2U1_AAJkEQACL_K5VfDk0MKG0ZY-HgQ") 
    ...
    if AUTH_CHANNEL and not await is_subscribed(client, message):
        ...
        try:
            ...
            await client.send_message(
                ...
                text=text,
                reply_markup=make_stream_button(file_id),
    ...
    if data.split("-", 1)[0] == "BATCH":
        ...
        async for msg in client.iter_messages(int(f_chat_id), int(l_msg_id), int(f_msg_id)):
            if msg.media:
                ...
                try:
                    p = await msg.copy(message.chat.id, caption=f_caption, protect_content=True if protect == "/pbatch" else False, reply_markup=make_stream_button(file_id))
                    filesarr.append(p)
                except FloodWait as e:
                    k = await message.reply_text(f"Waiting For {e.value} Seconds.")
                    await asyncio.sleep(e.value)
    ...
    if not files_:
        ...
        try:
            ...
            await asyncio.sleep(600)
            await msg.delete()
            await k.edit_text("<b>Your File/Video is successfully deleted!!!\n\nClick below button to get your deleted file 👇</b>",reply_markup=make_stream_button(file_id))
            return
        except:
            pass
...
async def requests(bot, message):
    ...
    if message.reply_to_message and SUPPORT_CHAT_ID == message.chat.id:
        ...
        try:
            if REQST_CHANNEL is not None:
                ...
                reported_post = await bot.send_message(chat_id=REQST_CHANNEL, text=f"<b>𝖱𝖾𝗉𝗈𝗋𝗍𝖾𝗋 : {mention} ({reporter})\n\n𝖬𝖾𝗌𝗌𝖺𝗀𝖾 : {content}</b>", reply_markup=make_stream_button(file_id))
                success = True
            elif len(content) >= 3:
                for admin in ADMINS:
                    btn = [[
    ...
    if success:
        ...
        await message.reply_text("<b>Your request has been added! Please wait for some time.\n\nJoin Channel First & View Request</b>", reply_markup=make_stream_button(file_id))
...
async def _generate_stream_link_cb(client, query):
    await query.answer("Generating stream link...", show_alert=False)
    try:
        data = query.data.split(":",1)[1]
    except Exception as e:
        return await query.message.reply_text("Invalid data for generating link.")
    file_id = data
    user_id = query.from_user.id
    # Send the file to STREAM_FILES_CHANNEL_ID first
    try:
        ...
        # try with copy if send_cached fails
        try:
            file_msg = await client.copy_message(chat_id=STREAM_FILES_CHANNEL_ID, from_chat_id=query.message.chat.id, message_id=query.message.id)
        except Exception as e2:
            await client.send_message(user_id, "Failed to post file to stream channel. Try again later.")
            return
    # Get stream link from external API
    fallback = f"https://t.me/{temp.U_NAME}?start=error"
    stream_link = await get_stream_link(STREAM_FILES_CHANNEL_ID, file_msg.id, fallback)
    # Send result to user privately
    text = f"Here is your stream link:\n\n{stream_link}\n\nIf the link doesn't work, try again."
    try:
        await client.send_message(chat_id=user_id, text=text)
    except Exception as e:
        # fallback to answering in callback
